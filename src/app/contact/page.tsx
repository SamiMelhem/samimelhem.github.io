...
export default function ContactPage() {
  ...
  return (
    <main className="relative min-h-screen flex flex-col items-center justify-center text-white pb-32 px-4">      
      ...
      >
        <h1 className="text-4xl sm:text-5xl md:text-6xl text-center font-bold mb-8 sm:mb-12 underline decoration-teal-400">
            {`Contact Me`}
        </h1>
        <p className="text-base sm:text-lg text-gray-300 max-w-xl px-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 md:gap-8 w-full">
          {contacts.slice(0, 2).map(({ href, label, Icon, external }) => (
            <Link
              key={label}
              href={href}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 md:gap-8 w-full">
          {contacts.slice(2, 4).map(({ href, label, Icon, external }) => (
            <Link
              key={label}
              href={href}
              {...(external ? { target: '_blank' } : {})}
